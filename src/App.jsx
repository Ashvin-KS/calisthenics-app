  ...
  const [completedExercises, setCompletedExercises] = useState(() => {
    ...
    Object.keys(workoutData).forEach(day => {
      workoutData[day].exercises.forEach((exercise, exerciseIndex) => {
        ...
        if (!initial[key] || initial[key].length !== exercise.sets) {
  ...
  return (
    <div className="app-container"> {/* New container for main content + sidebar */}
                    ...
                    >
                          ...
                          <div className="exercise-meta">
                            ...
                            <span className="exercise-reps">
                              ...
                              <div className="rep-adjust-controls">
                                <button onClick={(e) => { e.stopPropagation(); handleRepAdjustment(exerciseIndex, -1); }}>-</button>
                                <button onClick={(e) => { e.stopPropagation(); handleRepAdjustment(exerciseIndex, 1); }}>+</button>
                              </div>
                            </span>
                          </div>
                      ...
                      <div className="set-completion-bars">
                        {Array.from({ length: exercise.sets }).map((_, setIndex) => (
                          <button
                            key={setIndex}
                            className={`set-bar ${completedExercises[`${currentDay}-${exerciseIndex}`]?.[setIndex] ? 'completed' : ''}`}
      ...
      <div className={`right-sidebar ${selectedVideo ? 'active' : ''}`}>
        {selectedVideo && (
          <>
            <button className="sidebar-close-btn" onClick={() => setSelectedVideo(null)}>
              <X size={24} />
            </button>
            <iframe
              width="100%"
